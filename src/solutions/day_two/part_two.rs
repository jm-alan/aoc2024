...
static mut DAMPENED: [i8; 7] = [0; 7];
static mut CURRENT_LEN: usize = 0;

pub fn solution() -> i64 {
  UNSAFE_REPORTS.iter().fold(0, |sum, report| {
    return is_safe_dampened(report) + sum;
  })
}

#[inline(always)]
fn is_safe_dampened(report: &'static [i8]) -> i64 {
  unsafe {
    CURRENT_LEN = report.len();
  }

  for skip in 0..unsafe { CURRENT_LEN } {
    for idx in 0..skip {
      unsafe {
    ...
    for idx in (skip + 1)..unsafe { CURRENT_LEN } {
      unsafe {
        DAMPENED[idx - 1] = report[idx];
      }
    }

    if is_safe(&unsafe { DAMPENED }[0..(unsafe { CURRENT_LEN } - 1)]) {
      return 1;
    }
  }
