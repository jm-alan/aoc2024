...
static mut SET_ONCE: bool = false;
static mut INCREASING: bool = false;

pub fn is_safe(report: &[i8]) -> bool {
  unsafe { SET_ONCE = false };
  ...
  for idx in 1..report.len() {
    ...
    match (unsafe { SET_ONCE }, unsafe { INCREASING }, diff) {
      (true, false, 1..=i8::MAX) | (true, true, i8::MIN..=-1) => return false,
      (false, _, _) => unsafe {
        SET_ONCE = true;
        INCREASING = diff > 0;
      },
